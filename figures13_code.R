...
PANEL_figures3D_ZOOM=function(show_z_scale){
	...
	library(ggplot2); library(grid); library(xlsx); library(gridExtra); library(ggpubr)

	#grid.newpage()

	if(show_z_scale==TRUE){
		data=read.xlsx("../../PANEL_STUDY_REPORT/Tables_25apr18.xlsx",encoding = "UTF-8",5)
		for(x in 3:dim(data)[2]){ data[,x]=as.numeric(data[,x]) }
	}else{ data=read.xlsx("../../PANEL_STUDY_REPORT/Tables_25apr18.xlsx",encoding = "UTF-8",4) }

	my_cols=c("nasal.Bet.v.1.specific.IgA","nasal.Bet.v.1.specific.IgG4","nasal.Bet.v.1.specific.IgG4","nasal.IgA","nasal.FLC.L","nasal.FLC.K","nasal.IL.33","nasal.MIP.1ß","nasal.Eotaxin.2.CCL24","nasal.Eotaxin.2.CCL24","nasal.Eotaxin.2.CCL24","nasal.Eotaxin.2.CCL24","nasal.IL.1ß","nasal.MCP.1.CCL2","nasal.MCP.1.CCL2")
	...
	for(x in 1:length(my_cols)){
		...
		A=data[ix1,my_cols[x]]; A=A[!is.na(A)]; my_pat=paste("NA.*",to_sel,"$",sep=""); ix2=(grep(my_pat,as.character(data[,2])))
		B=data[ix2,my_cols[x]]; B=B[!is.na(B)]; B=as.numeric(B); A=as.numeric(A)
...
PANEL_supplemental_figure1=function(plot_all=FALSE){
	...
	if(plot_all){ par(mfrow=c(5,5)) }
	else{	par(mfrow=c(6,4)) }

	# ------------------------------------------------------------------------------
	table_file="../../PANEL_STUDY_REPORT/Tables_1may18.xlsx"
	# ------------------------------------------------------------------------------
	...
	nasal.FLC=c()
	for(x in 1:dim(dd)[1]){
		arr=c(dd[x,"nasal FLC L"],dd[x,"nasal FLC K"])
		arr=arr[!is.na(arr)]
	...
	for(my_cn in 3:length(cn)){	
		...
		for(p in 1:length(X)){
			...
			my_sym_in=df_filt[,"Symptom..birch."]
			my_sym_out=df_filt[,"Symptom.birch.out.of.season."]
		...
		c_y_max=c(df[,"Symptom..birch."],df[,"Symptom.birch.out.of.season."]); c_y_max=c_y_max[!is.na(c_y_max)]; c_y_max=max(c_y_max)
...
# helper function of figure 3a
PANEL_figure3B=function(req_samples=5){

	library(pheatmap); library(xlsx);
	options(stringsAsFactors=FALSE)

	do_normalise=function(){
		data=read.xlsx("../../PANEL_STUDY_REPORT/Tables_30apr18.xlsx",encoding = "UTF-8",4); data2=data
		...
		normalize_data=function(data){
			...
			for(s in 3:dim(data)[2]){
				for(p in 1:length(pat)){
					...
					my_val=data[ix,s]; my_val=as.double(as.character(my_val)); my_val=my_val[!is.na(my_val)]
					if(length(my_val)<=req_samples){ data[ix,s]=NA } else{ data[ix,s]=(data[ix,s]-mean(my_val))/(sd(my_val))}
				}
			}
			return(data)
		}
		data=normalize_data(data); write.table(data,"val_ztrans.txt"); write.table(data2,"raw_vals.txt")
	}
	do_normalise()
	...
	calc_log_change=function(c_f){
		...
		print(data)
		df_f_matx=data.frame(); df_p_matx=data.frame()
		...
		for(y in 3:dim(data)[2]){
			...
			for(x in 1:15){
				...
				if(length(D1)>0 && length(D2)>0){
					...
					arr_f=c(arr_f,NA); arr_p=c(arr_p,NA)
				}
			}
			df_f_matx=rbind(df_f_matx,arr_f); df_p_matx=rbind(df_p_matx,arr_p)
		}	
	...
	do_convert=function(df_f_matx, df_p_matx){
		c_n2=c("serum.Bet.v.1.specific.IgE","serum.Bet.v.1.specific.IgG4","serum.Bet.v.1.specific.IgA")
		M_f=df_f_matx[c_n2,]; M_f=as.matrix(df[c_n2,])
		M_f2=df_f_matx[!(rownames(df_f_matx)%in%(c_n2)),]; M_f2=as.matrix(df_f_matx[!(rownames(df_f_matx)%in%(c_n2)),])
		M_p=df_p_matx[c_n2,]; M_p=as.matrix(df[c_n2,])	
...
# actual figure 3 uses PANEL_figure3B
PANEL_figure3=function(opt){
	...
	if(opt=="nasal"){	Mr=M2_r[dim(M2_r):1,]; Mpr=M2p_r[dim(M2p_r):1,]; Mnr=M2_nr[dim(M2_nr):1,]; Mpnr=M2p_nr[dim(M2p_nr):1,] }
	else if(opt=="serum"){ Mr=M1_r[dim(M1_r):1,]; Mpr=M1p_r[dim(M1p_r):1,]; Mnr=M1_nr[dim(M1_nr):1,]; Mpnr=M1p_nr[dim(M1p_nr):1,] }
...
PANEL_figure2NEW=function (my_opt) 
{
	...
	cor_method="pearson"; my_genome="birch"; prick_type="whole_season"; sel_opt="all"
	options(stringsAsFactors=FALSE)

	data=read.xlsx("../../PANEL_STUDY_REPORT/Tables_1may18.xlsx",3)
	print(colnames(data))
	...
	if(my_genome=="birch"){
		for(mo in my_opt){
			if(sel_opt=="nose"){
				label2A="Symptom..birch..nose."; label2B="Symptom.birch.nose.out.of.season."; label2_show="symptom nose (avg)"
			}else{
				label2A="Symptom.birch.in.season."; label2B="Symptom.birch.out.of.season."; label2_show="symptom (avg)"
			}
			label1A="Pricktest..wheal..in.season."; label1B="Pricktest..wheal..out.of.season."
			if(mo=="all"){
				my_col=c()
				for(x in 1:dim(df)[1]){
	...
	if(my_genome=="birch"){
		for(mo in my_opt){
			...
			label1A="Pricktest..flear..in.season."; label1B="Pricktest..flear..out.of.season."
			if(mo=="all"){
				my_col=c()
				for(x in 1:dim(df)[1]){
...
PANEL_figure1=function (my_opt) 
{
	...
	man=readPNG("../../PANEL_STUDY_REPORT/previous_analysis_before_24apr18/old/man.png"); 
	woman=readPNG("../../PANEL_STUDY_REPORT/previous_analysis_before_24apr18/old/woman.png")
	...
	for(mo in 1:length(my_opt)){
		...
		data=read.xlsx("../../PANEL_STUDY_REPORT/Tables_1may18.xlsx",2)
